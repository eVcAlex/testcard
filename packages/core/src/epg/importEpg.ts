 ...
 *
 * The caller (the desktop main process) fetches the URL and hands the response body in — this
 * function never does I/O of its own, so `packages/core` keeps its no-Node-HTTP stance and the
 * credential-bearing Xtream `xmltv.php` URL is built and used only in main.
 *
...
export async function importEpg(
  ...
  options: { readonly gzipped?: boolean; readonly onProgress?: (programmesSoFar: number) => void } = {},
