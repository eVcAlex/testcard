...
/**
 * One choice in a horizontal row (the Live TV categories). Quiet at rest, solid white when it is the open
 * one, ringed under the remote's focus. Handlers take the pill's `id` so a row can share two functions and
 * let `memo` work.
 */
...
const styles = styleSheet({
  ...
  focused: { backgroundColor: colors.cardActive, borderColor: colors.foreground },
  label: { color: colors.muted, fontSize: 24, fontWeight: "400" },
  labelActive: { color: colors.background, fontWeight: "500" },
  labelFocused: { color: colors.foreground, fontWeight: "500" },
});
