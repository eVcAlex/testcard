 ...
 * A long list scrolls. `aside` is drawn beside the list (a preview of the option `onFocusId` last reported).
 */
      ...
      <TVFocusGuideView autoFocus trapFocusUp trapFocusDown trapFocusLeft trapFocusRight style={styles.panel}>
        ...
        <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
          {options.map((option, index) => (
            <MenuRow key={option.id} id={option.id} label={option.label} active={option.id === preferredId} preferred={preferredId !== undefined && options.some((entry) => entry.id === preferredId) ? option.id === preferredId : index === 0} onPressId={choose} onFocusId={onFocusId} />
          ))}
        </ScrollView>
      </TVFocusGuideView>
    </View>
...
const styles = styleSheet({
  scrim: { position: "absolute", left: 0, right: 0, top: 0, bottom: 0, zIndex: 30, backgroundColor: "#000000a6", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 48 },
