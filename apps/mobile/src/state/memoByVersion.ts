...
export function memoByVersion<Db extends object, T>(
...
): ((db: Db, version: number, key?: string) => T) & { cached: (db: Db, version: number, key?: string) => T | undefined } {
  ...
  return Object.assign(read, { cached: (db: Db, version: number, key?: string) => values(db, version).get(key ?? "") });
}
