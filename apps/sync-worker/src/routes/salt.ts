...
export async function handleGetSalt(c: AppContext): Promise<Response> {
...
export async function handleSetSalt(c: AppContext): Promise<Response> {
  const parsed = await parseJsonBody(c, SetSaltBodySchema);
  if (!parsed.ok) return parsed.response;
  const { salt } = parsed.data;
